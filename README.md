...
데모마다 원본 비율을 그대로 씁니다 (21:9, 16:9, 4:3 섞여 있어도 잘리지 않습니다).

남의 영상은 파일을 받아서 올리지 말고 `media` 대신 `youtube` 에 영상 ID 만 적으세요.
플랫폼 플레이어로 임베드되어 저작권·출처 문제가 생기지 않습니다.

```json
{ "title": "...", "project": "...", "youtube": "-7BEzmdrlcU", "description": "..." }
```
